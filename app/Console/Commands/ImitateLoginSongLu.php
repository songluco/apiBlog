...
class ImitateLoginSongLu extends Command
{
    protected $url = 'http://songlu.login.com/login';

    protected $email = 'pconsequatur@example.net';
     ...
     * @var string
     */
    protected $signature = 'command:ImitateLoginSongLu';
     ...
     * @var string
     */
    protected $description = '模拟登陆SongLu的login网站';
    ...
    {
        $url = 'http://songlu.login.com/home';
        $curl = new Curl();
        $csrf = file_get_contents($this->cookie_xsrf_file);
        $laravel = file_get_contents($this->cookie_laravel_file);
        $curl->setCookie('XSRF-TOKEN', $csrf);
